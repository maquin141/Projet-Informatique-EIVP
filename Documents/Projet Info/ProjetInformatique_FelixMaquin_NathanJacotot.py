...
def horaires():
    ...
    if len(I)==0:
        w1=0
        while w1<len(A):
            if '07:45:00'<A[w1]<'09:40:00' and int(B[w1])>=108:#si la liste I est vide, on enlève la contrainte de l'augmentation du bruit d'une heure à la suivante et on réduit la contrainte sur les 120 lux de 10%
                P.append(C[w1])
                G.append(A[w1])
            w1=w1+1
    else:
